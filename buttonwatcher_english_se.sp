...
public void Button_Pressed(const char[] output, int caller, int activator, float delay)
{
	...
	if(GetConVarBool(cv_showtype)) 
	{
		...
		{
			PrintToChatAll(" \x02%s \x0C%N \x04pressed the button\x0C %i %s", seHeader, activator, caller, entity);
		}
		else 
		{
			...
			{	
				...
				{
					PrintToChat(i, " \x02%s \x0C%N \x04pressed the button\x0C %i %s", seHeader, activator, caller, entity);
				}
			}
		}
	}
	else
	{
		if(!GetConVarBool(cv_onlyadmins)) 
		{
			PrintToConsoleAll("%s %N pressed the button %i %s", seHeader, activator, caller, entity);
		}
		else 
		{
			for (int i = 1; i <= MaxClients; i++)
			{	
				if (IsClientInGame(i) && (GetUserAdmin(i) != INVALID_ADMIN_ID || IsClientSourceTV(i)))
				{
					PrintToConsole(i, "%s %N pressed the button %i %s", seHeader, activator, caller, entity);
				}
			}
		}
	...
	if(GetConVarBool(cv_log)) 
		LogMessage("%s %L pressed the button %i %s", seHeader, activator, caller, entity)
